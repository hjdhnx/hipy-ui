<template>
  <div class="detail-container">
    <div class="player-wrap">
      <div class="left">
        <video v-if="!playUrl" id="player-box" />
        <video-player
          v-if="playUrl"
          ref="videoPlayer"
      ...
      <div v-for="(tab_item, tab_index) in tData.playData" :key="tab_index" class="right">
        ...
        <div class="p-list">
          <div
            ...
            :key="item.label"
            class="p-item"
            :class="currentLink===item.link? 'active':''"
            @click="startPlay(item.link,tData.tabList[tab_index])"
          >{{ item.label }}
          </div>
        </div>
      </div>
      ...
      <div class="meta-wrap">
        ...
        <div class="info-wrap">
          ...
          <div class="info">
            ...
            <div class="info-item">
              <div v-html="tData.detailData.brief" />
            </div>
          </div>
        </div>
      </div>
      <h3 style="height:40px;border-bottom: 1px solid rgba(0, 0, 0, 0.04);">内容详情</h3>
      <div v-html="tData.detailData.vod_content" />

      <h3 style="height:40px;border-bottom: 1px solid rgba(0, 0, 0, 0.04);">为你推荐</h3>
      <div class="lvideo-list">
        <a v-for="item in tData.recommendData" :key="item.vod_id" class="video-item" :href="handleDetail(item.vod_id)">
  ...
  methods: {
    ...
    startPlay(url, flag) {
      ...
      PlayApi(url, flag).then(resp => {
        console.log(resp)
        const $player = document.querySelector('video[id="player-box"]')
        if ($player) {
          $player.setAttribute('src', resp.url)
          $player.setAttribute('autoplay', true)
        }
        const playUrl = resp.url
    ...
    setSEO() {
      const title = this.tData.detailData.vod_name
      const director = this.tData.detailData.vod_director
      const actor = this.tData.detailData.vod_actor
      if (title) {
        const seoTitle = title + '-免费观看-在线电影免费'
        const seoKeywords = title + ',在线观看,' + director
        const seoDescription = title + ',在线观看,' + actor
    ...
    getData() {
      DetailApi(this.vod_id).then(res => {
        if (res.list && res.list.length > 0) {
          ...
          // 剥离播放地址
          const playData = []
          const tabList = this.tData.detailData.vod_play_from.split('$$$')
          const vodPlayUrl = this.tData.detailData.vod_play_url
          const vodTabList = vodPlayUrl.split('$$$')
          vodTabList.forEach(tab_item => {
            const vodPlayList = tab_item.split('#')
            const tabPlayData = []
            vodPlayList.forEach(item => {
              const playItem = item.split('$')
              const label = playItem[0]
              const link = playItem[1]
              tabPlayData.push({
                label: label,
                link: link
              })
